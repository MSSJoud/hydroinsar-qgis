...
Run the script with:
    python3 tools/w3ra_mat_2netcdf.py --input-dir /path/to/matfiles --latlon-file /path/to/LatLon.mat --output-file /path/to/output.nc
...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert W3RA yearly .mat files into a single NetCDF product.")
    parser.add_argument("--input-dir", required=True, help="Directory containing yearly W3RA .mat files.")
    parser.add_argument("--latlon-file", required=True, help="Path to LatLon.mat containing lat_EU and lon_EU.")
    parser.add_argument("--output-file", required=True, help="Path to output NetCDF file.")
    parser.add_argument(
        "--extract-sg",
        action="store_true",
        help="Also extract Sg_EU_* variables into a separate NetCDF file using ncks/ncdump if available.",
    )
    return parser.parse_args()


def extract_sg_variables(output_file):
    sg_output_file = output_file.replace(".nc", "_Sg_EU.nc")
    extract_command = (
        f"ncks -v $(ncdump -h {output_file} | grep Sg_EU | awk '{{print $1}}' | "
        f"grep -o 'Sg_EU_[0-9]*' | tr '\\n' ',') -O {output_file} {sg_output_file}"
    )
...
if __name__ == "__main__":
    args = parse_args()
    convert_w3ra_mat_to_netcdf(args.input_dir, args.latlon_file, args.output_file)
    if args.extract_sg:
        extract_sg_variables(args.output_file)
